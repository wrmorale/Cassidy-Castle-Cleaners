...
public class RotateToMovement : ActionNode
{
    ...
    protected override State OnUpdate() {
        ...
        {
            Quaternion newRotation = Quaternion.LookRotation(context.enemy.movement, context.characterController.transform.up);
            // Set x and z rotation to zero
            newRotation = Quaternion.Euler(0f, newRotation.eulerAngles.y, 0f);
            context.characterController.transform.rotation = Quaternion.Slerp(context.characterController.transform.rotation, newRotation, Time.fixedDeltaTime * context.enemy.rotationSpeed * speedMultiplier);
